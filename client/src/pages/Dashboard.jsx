...
function Dashboard() {
  ...
  const fetchDecks = async () => {
    try {
      const res = await axios.get(`http://localhost:5000/api/pitch/my-decks/${user._id || user.id}`)
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-white text-2xl font-bold">PitchAI Dashboard</h1>
        ...
        <div className="grid grid-cols-3 gap-4 mb-10">
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6 text-center">
            <div className="text-4xl font-bold text-purple-400">{decks.length}</div>
            <div className="text-gray-400 mt-1">Total Decks</div>
          </div>
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6 text-center">
            <div className="text-4xl font-bold text-green-400">{decks.length * 10}</div>
        ...
        <h2 className="text-white font-bold text-xl mb-4">📁 Mere Pitch Decks</h2>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-white/5 rounded-2xl border border-white/10">
            ...
            <p className="text-gray-400 text-lg mb-6">Abhi koi deck nahi hai!</p>
            <Link
              to="/create"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {decks.map((deck) => (
              <div key={deck.shareId} className="bg-white/5 border border-white/10 rounded-2xl p-6 hover:bg-white/10 transition">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h3 className="text-white font-bold text-lg">{deck.startupName}</h3>
                    <p className="text-gray-400 text-sm">{deck.industry} • {deck.fundingGoal}</p>
                ...
                <div className="flex gap-2 mt-4">
                  ...
                  <button
                    onClick={() => {
                      navigator.clipboard.writeText(`http://localhost:5173/deck/${deck.shareId}`)
